 ...
 * 
 * > wsimport -keep http://localhost:8080/HinemosWS/RepositoryEndpoint?wsdl -s
 * src\main\ws
 * 
 ...
 *
 */
public class JAXWSStub2 extends ServerResource {

	private static final String HINEMOS_MANGER_URL = System.getenv("HINEMOS_MANGER_URL");
	private static final String WSDL_URI = (null!=HINEMOS_MANGER_URL) ? HINEMOS_MANGER_URL+"RepositoryEndpoint?wsdl" : null;
	...
	private void setBindingProvider(Object o) {
	...
	public List<RepositoryTableInfo> getPlatformList() {
		try {
			RepositoryEndpointService service;
			if(null!=WSDL_URI)
				service = new RepositoryEndpointService(new URL(WSDL_URI));
			else
				service = new RepositoryEndpointService();
			RepositoryEndpoint port = service.getRepositoryEndpointPort();
			
			setBindingProvider(port);
			return port.getPlatformList();
		} catch (MalformedURLException | HinemosUnknown_Exception | InvalidRole_Exception | InvalidUserPass_Exception e) {
			e.printStackTrace();
		}
		return null;
	}
	...
	public static void main(String[] args) {
		System.out.println(new JAXWSStub2().getPlatformList());
	}
}
